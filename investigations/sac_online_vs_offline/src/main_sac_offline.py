...
if __name__ == "__main__":
    # 1. Load your recorded transitions
    with open('lunarlander_online_dataset.pkl', 'rb') as f:
        dataset = pickle.load(f)

    # 2. Create env & SAC agent
    ...
    # 3. Pre-fill the replay buffer
    for obs, act, rew, obs_next, done in dataset:
        agent.remember(obs, act, rew, obs_next, done)

    # 4. Offline update loop
    n_train_steps = 50_000      # lower this for quick tests
    eval_interval = 5_000
    eval_scores = []
