...
if __name__ == '__main__':
    ...
    vae = VAE(input_dim=8, latent_dim=6)
    vae.load_state_dict(T.load("models/vae_state_encoder_6st.pt"))
    vae.to(device)
    vae.eval()
    ...
    np.save(os.path.join(results_dir, 'scores_6st.npy'), np.array(scores))
    np.save(os.path.join(results_dir, 'steps_6st.npy'),  np.array(steps))
