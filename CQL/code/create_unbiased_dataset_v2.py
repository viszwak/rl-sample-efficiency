...
class UnbiasedDatasetGenerator:
    ...
    def collect_episode(self, policy_fn, policy_name, max_steps=1000):
        if self.buffer.mem_cntr >= self.buffer.mem_size:
            ...
            return 0, 0  # Return early if buffer is full
    ...
    def generate_dataset(self, target_transitions=1000000):
        ...
        print(f"Generating dataset with per-transition policy selection...")
        
        policies = [
            ("expert_perfect", lambda s: self.noisy_expert_policy(s, 0.0), 0.1),      # 10%
            ("expert_noisy", lambda s: self.noisy_expert_policy(s, 0.1), 0.025),        # 2.5%
            ("conservative", self.conservative_policy, 0.2),                          # 20%
            ("failing", self.failing_policy, 0.2),                                    # 20%
            ("exploratory", self.exploratory_policy, 0.2),                           # 20%
            ("random", self.random_policy, 0.2),                                     # 20%
            ("expert_high_noise", lambda s: self.noisy_expert_policy(s, 0.2), 0.025),   #2.5%
            ("mixed", self.mixed_policy, 0.05),                                       # 5%
        ]
        ...
        while self.buffer.mem_cntr < target_transitions:
            ...
            self.episode_returns.append(episode_return)
            self.trajectory_lengths.append(episode_length)  # Track this!
            self.total_episodes += 1
            
            if self.total_episodes % 50 == 0:
                print(f"Episodes: {self.total_episodes}, Transitions: {self.buffer.mem_cntr}/{target_transitions}")
    ...
    def perform_state_clustering(self, n_samples=10000):
        """Perform clustering on state space for better analysis"""
    ...
    def analyze_dataset(self):
        ...
        # Get policy IDs from our separate tracking
        policy_ids = self.policy_memory[:data_size] if self.policy_memory else []

        # Clip actions for analysis (single clipping operation)
        ...
        # Perform clustering-based state coverage analysis
        ...
        # Fixed trajectory length statistics
        ...
        # Clustering analysis
        if data_size >= 100:  # Only perform clustering if we have enough data
    ...
    def visualize_dataset(self):
        ...
        plt.close(fig)
        print("Visualization saved to 'unbiased_dataset_analysis.png'")
